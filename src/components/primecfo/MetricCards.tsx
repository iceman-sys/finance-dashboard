...
import {
  DollarSign, CreditCard, TrendingUp, TrendingDown, PieChart, Wallet, FileText, ArrowUpRight, ArrowDownRight, Minus
} from 'lucide-react';
import { MetricCard, formatCurrency, formatPercentChange, getPercentChange } from '@/lib/financialData';
...
const iconMap: Record<string, React.FC<{ className?: string }>> = {
  DollarSign, CreditCard, TrendingUp, PieChart, Wallet, FileText,
};
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {metrics.map((metric) => {
        ...
        } else if (metric.format === 'days') {
          displayValue = `${metric.value} days`;
        } else {
          ...
          >
            ...
            <div className="mt-3 pt-3 border-t border-slate-700/30">
              <p className="text-xs text-slate-500">
                Previous: {metric.format === 'currency' ? formatCurrency(metric.previousValue) : metric.format === 'percentage' ? `${metric.previousValue}%` : metric.previousValue.toLocaleString()}
              </p>
            </div>
          </div>
        );
