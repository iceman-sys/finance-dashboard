...
import {
  LayoutDashboard, FileText, Brain, Users, Link2, Settings,
  ChevronDown, Building2, RefreshCw, HelpCircle
} from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({
  currentView, onNavigate, selectedClient, clients, onSelectClient, isOpen, onClose
}) => {
  ...
  return (
    <>
      ...
      <aside className={`fixed top-16 left-0 h-[calc(100vh-4rem)] w-64 bg-slate-900 border-r border-slate-800 z-40 transition-transform duration-300 ${
                ...
                <div className="flex-1 text-left min-w-0">
                  ...
                  {selectedClient && (
                    <div className="flex items-center gap-1.5">
                      <div className={`w-1.5 h-1.5 rounded-full ${qbStatusColors[selectedClient.qbStatus]}`} />
                      <span className="text-xs text-slate-400 capitalize">{selectedClient.qbStatus}</span>
                    </div>
                  )}
                </div>
                <ChevronDown className={`w-4 h-4 text-slate-400 transition-transform ${clientDropdownOpen ? 'rotate-180' : ''}`} />
              </button>
                ...
                <div className="absolute top-full left-0 right-0 mt-2 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl overflow-hidden z-50">
                  <div className="max-h-64 overflow-y-auto">
                    {clients.filter(c => c.status === 'active').map((client) => (
                      <button
                        key={client.id}
                        onClick={() => { onSelectClient(client); setClientDropdownOpen(false); }}
                      ...
                      >
                        ...
                        <div className="text-left min-w-0">
                          ...
                          <p className="text-xs text-slate-400">
                            {client.qbStatus === 'connected' ? `Synced ${timeAgo(client.lastSync)}` : client.qbStatus}
                          </p>
                        </div>
                      </button>
                    ))}
