...
const Navbar: React.FC<NavbarProps> = ({ currentView, onNavigate, isLoggedIn, onLogin }) => {
  ...
  const notifications = [
    { id: 1, text: 'GreenLeaf Landscaping sync completed', time: '23m ago', read: false },
    { id: 2, text: 'New AI insights available for TechPulse', time: '1h ago', read: false },
    { id: 3, text: 'Blossom Café connection expired', time: '6h ago', read: true },
  ];
  ...
  return (
    <nav className="bg-slate-900 border-b border-slate-800 sticky top-0 z-50">
                ...
                <div className="relative">
                  ...
                  {notifOpen && (
                    <div className="absolute right-0 mt-2 w-80 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl overflow-hidden">
                      ...
                      {notifications.map((n) => (
                        <div key={n.id} className={`px-4 py-3 border-b border-slate-700/50 hover:bg-slate-700/50 cursor-pointer ${!n.read ? 'bg-slate-700/20' : ''}`}>
                          <p className="text-sm text-slate-200">{n.text}</p>
                          <p className="text-xs text-slate-500 mt-1">{n.time}</p>
                        </div>
                      ))}
                      <div className="px-4 py-2">
                        <button className="text-xs text-teal-400 hover:text-teal-300">View all notifications</button>
                      </div>
                    </div>
                  )}
                </div>
                ...
                <div className="relative">
                  ...
                  {profileOpen && (
                    <div className="absolute right-0 mt-2 w-56 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl overflow-hidden">
                      ...
                      <div className="py-1">
                        <button onClick={() => { onNavigate('settings'); setProfileOpen(false); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-slate-300 hover:text-white hover:bg-slate-700">
                          <Settings className="w-4 h-4" /> Settings
                        </button>
                        <button onClick={() => { onNavigate('clients'); setProfileOpen(false); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-slate-300 hover:text-white hover:bg-slate-700">
                          <User className="w-4 h-4" /> Manage Clients
                        </button>
                        <button onClick={() => { onLogin(); setProfileOpen(false); }} className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-red-400 hover:text-red-300 hover:bg-slate-700">
        ...
        <div className="md:hidden border-t border-slate-800 bg-slate-900">
          <div className="px-4 py-3 space-y-1">
            {isLoggedIn ? (
              ...
              <>
                <button onClick={() => { onLogin(); setMobileMenuOpen(false); }} className="block w-full text-left px-4 py-2.5 text-sm text-slate-300 hover:bg-slate-800 rounded-lg">
                  Sign In
                </button>
                <button onClick={() => { onLogin(); setMobileMenuOpen(false); }} className="block w-full text-left px-4 py-2.5 text-sm text-teal-400 font-medium hover:bg-slate-800 rounded-lg">
                  Get Started Free
                </button>
              </>
            )}
      ...
      {(profileOpen || notifOpen) && (
        <div className="fixed inset-0 z-[-1]" onClick={() => { setProfileOpen(false); setNotifOpen(false); }} />
      )}
    </nav>
  );
};
